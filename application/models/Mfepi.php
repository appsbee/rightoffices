...
class Mfepi extends CI_Model {
	...
	public function check_existing_centre($centreid) {
		// return $this->db->select('count(*) as exist', false)->where('CentreID', $centreid)->get('Centre')->row_array();
		return $this->db->where('CentreID', $centreid)->count_all_results('Centre');
	}
	...
	public function set_existing_data($xml) {
		...
		foreach ($xml as $Centre => $CentreArray) {
			foreach ($CentreArray as $key => $CDataArray) {
				...
				if ($this->check_existing_centre($CDataArray['CentreID'])) {
					$tempData = $CDataArray;
					unset($tempData['OfficeTypes']);unset($tempData['Photos']);
					$centredata[] = $tempData;
